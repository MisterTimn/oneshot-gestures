...
import augmentation_v2 as aug
import convnet_19_augm as cnn
import load_class
...
def worker_backprop(q,samples,labels,indices_train):
    ...
    sharedSampleArray = sa.attach("shm://samples-augm")
    sharedLabelArray = sa.attach("shm://labels-augm")
...
if __name__=='__main__':
    try:
        sharedSampleArray = sa.create("shm://samples-augm", (BATCH_SIZE, 12, 64, 64), dtype='float32')
        sharedLabelArray = sa.create("shm://labels-augm", BATCH_SIZE, dtype='int32')
        ...
        for ONESHOT_CLASS in xrange(5):
            ...
            try:
                ...
                OUTPUT_DIRECTORY = "{}output/{}/excluding-{}-augm/".format(BASE_DIR, MODEL_EXCLUDING, ONESHOT_CLASS)
