...
def worker_backprop(q,samples,labels,indices_train,indices_train_oneshotclass,indices_train_oneshotclass_2):
...
if __name__=='__main__':
    ...
    try:
        ...
        for ONESHOT_CLASS in [13]:
            ...
            indices_train_oneshotclass_2 = indices_train[NUM_CLASSES - 2]


            proc = mp.Process(target=worker_backprop,
                              args=[q,samples,labels,indices_train,indices_train_oneshotclass,indices_train_oneshotclass_2])
            proc.daemon = True
            proc.start()

            for num_oneshot_samples in [1,10,100]:
