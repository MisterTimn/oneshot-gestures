...
def worker_backprop(q,samples,labels,indices_train,indices_train_oneshotclass):
    ...
    sharedSampleArray = sa.attach("shm://samples3")
    sharedLabelArray = sa.attach("shm://labels3")
...
if __name__=='__main__':
    q = mp.JoinableQueue()
    try:
        sharedSampleArray = sa.create("shm://samples3", (BATCH_SIZE, 12, 64, 64), dtype='float32')
        sharedLabelArray = sa.create("shm://labels3", BATCH_SIZE, dtype='int32')
        ...
        for ONESHOT_CLASS in [15]:
            ...
            indices_train_oneshotclass = indices_train[ONESHOT_CLASS]
